...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ReceiverTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        [tableView registerNib:[UINib nibWithNibName:@"ReceiverTableViewCell" bundle:nil] forCellReuseIdentifier:CellIdentifier];
        cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    }
    
    UserInfo *info = self.list[indexPath.row];
    
    if (![info.logoUrl isEqualToString:@""]) {
        [cell.imgAvatar setImageWithResizeURL:info.logoUrl];
    }
    cell.recvName.text = info.displayName;
    cell.recvRole.text = [Common sharedManager].roleArray[[info.role integerValue]];
    
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.delegate && [self.delegate respondsToSelector:@selector(NIDropDown:selectIndex:)]) {
        [self.delegate NIDropDown:self selectIndex:indexPath.row];
    }
